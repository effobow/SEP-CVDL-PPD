...
def main():
    ...
    if args.dataset == "fer2013":
        ...
        train_dir = 'archive/FER2013/train/'
        test_csv = 'archive/FER2013/test_FER_labels.csv'
        test_dir = 'archive/FER2013/test/'
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0 
            torch.save(model.state_dict(), f'models/best_model_{args.model}_{args.dataset}.pth')
        else:
            patience_counter += 1
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'reports/loss_curve_{args.model}_{args.dataset}.png', dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'reports/accuracy_curve_{args.model}_{args.dataset}.png', dpi=300)
    plt.close()
    ...
    df.to_csv(f'reports/training_metrics_{args.model}_{args.dataset}.csv', index=False)
    ...
    summary_df.to_csv(
        f"reports/run_summary_{args.model}_{args.dataset}.csv",
        index=False
    )

    print("Saved files:")
    print(f"- reports/run_summary_{args.model}_{args.dataset}.csv")
    print(f"- models/best_model_{args.model}_{args.dataset}.pth")
    print(f"- reports/training_metrics_{args.model}_{args.dataset}.csv")
    print(f"- reports/loss_curve_{args.model}_{args.dataset}.png")
    print(f"- reports/accuracy_curve_{args.model}_{args.dataset}.png")
